...
const posts: { id: number; title: string; content: string; author: string; createdAt: string }[] = [];
...
app.get("/posts/:id", (req, res) => {
  ...
  if (!post) return res.status(404).json({ error: `Blog post with ID ${req.params.id} not found` });
  res.json(post);
});
